...
export async function sendConfirmationEmail({ parentName, studentName, parentEmail, instruments, selectedCourse, preferredSlot, orchestra }) {
  ...
  await transporter.sendMail({
    from: '"קונסרבטוריון הבית למוסיקה" <avigailregev@gmail.com>',
    to: parentEmail,
    subject: 'קיבלנו את הרשמתך לקונסרבטוריון',
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #6D28D9;">קונסרבטוריון הבית למוסיקה</h2>
        <p>שלום ${parentName},</p>
...
export async function sendAssignmentEmail({ parentName, studentName, parentEmail, teacher, assignedDay, assignedTime, orchestra }) {
  // ✅ תיקון: ולידציה לפני שליחה
  if (!teacher) {
    throw new Error('sendAssignmentEmail: חסר שם מורה');
  }
  if (!parentEmail) {
    throw new Error('sendAssignmentEmail: חסר אימייל הורה');
  }
  ...
  await transporter.sendMail({
    from: '"קונסרבטוריון הבית למוסיקה" <avigailregev@gmail.com>',
    to: parentEmail,
    subject: 'שובצת לקונסרבטוריון הבית למוסיקה',
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #6D28D9;">קונסרבטוריון הבית למוסיקה</h2>
        <p>שלום ${parentName},</p>

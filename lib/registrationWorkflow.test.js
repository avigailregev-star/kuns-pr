import { assignmentStatusLabel, needsAttention, isStudentHandled, missingStatusLabels, registrationStudentStats } from './registrationWorkflow';
...
test('handled requires paid assigned individual and assigned-or-not-required fixed lessons', () => {
  ...
  expect(isStudentHandled(student({ theory_not_required: true }, [staleTheory]))).toBe(false);
...
test('dashboard stats count students rather than lesson registration rows', () => {
  const groups = groupStudentRows([
    {
      id: 'main-a', student_name: 'תלמידה א', parent_phone: '0501111111',
      created_at: '2026-09-01', selected_course: 'פרטני', status: 'שובץ',
      registration_status: 'Confirmed', ensemble_not_required: true,
      theory_not_required: false,
    },
    {
      id: 'theory-a', linked_registration_id: 'main-a', student_name: 'תלמידה א',
      parent_phone: '0501111111', created_at: '2026-09-02',
      selected_course: 'תיאוריה', status: 'שובץ',
    },
    {
      id: 'main-b', student_name: 'תלמיד ב', parent_phone: '0502222222',
      created_at: '2026-09-03', selected_course: 'פרטני', status: 'בבדיקה',
    },
  ]);

  expect(registrationStudentStats(groups)).toEqual({
    total: 2,
    new: 0,
    reviewing: 1,
    assigned: 1,
...
test('assigned dashboard total requires every lesson assignment and confirmed payment', () => {
  const complete = student();
  const unpaid = student({ registration_status: 'Pending' });
  const missingTheory = student({ theory_not_required: false });

  expect(registrationStudentStats([complete, unpaid, missingTheory]).assigned).toBe(1);
});

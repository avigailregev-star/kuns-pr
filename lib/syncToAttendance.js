...
function toLessonType(selectedCourse) {
  ...
  if (c.includes('מנגינות')) return 'melodies_individual';
  if (c.includes('קבוצתי') || c.includes('קבוצ')) return 'group';
  return 'individual_60';
}

async function deactivateIfUnused(supabase, groupId, studentName, registrationId) {
  const { data: registrations, error } = await supabase.from('registrations')
    .select('id, status, registration_status')
    .eq('group_id', groupId)
    .eq('student_name', studentName);
  if (error) throw new Error(`Cannot verify remaining registrations: ${error.message}`);
  const stillAssigned = (registrations || []).some(row =>
    row.id !== registrationId && row.status === 'שובץ' && row.registration_status !== 'Cancelled'
  );
  if (!stillAssigned) {
    const { error: updateError } = await supabase.from('students')
      ...
      .eq('group_id', groupId)
      .eq('name', studentName);
    if (updateError) throw new Error(`Cannot deactivate attendance: ${updateError.message}`);
  }
}

export async function syncRegistrationToAttendance(supabase, reg, explicitGroupId) {
  const {
    id,
    teacher,
    assigned_day,
  ...
  if (excluded.includes(status) || registration_status === 'Cancelled') {
    if (currentGroupId && student_name) {
      await deactivateIfUnused(supabase, currentGroupId, student_name, id);
    }
    return;
  }

  let groupId = explicitGroupId || null;

  // When a group was already picked manually (e.g. "existing lesson" picker in
  // AdminTable), skip the day/time/name matching below entirely — searching
  // again with the registration's own fields can miss that group and spawn a
  // duplicate one.
  if (!groupId) {
    ...
    for (const g of (candidates || [])) {
      const match = (g.group_schedules || []).find(
        s => s.day_of_week === dayNum && s.start_time && s.start_time.startsWith(timePrefix)
      );
    ...
    if (!groupId) {
      ...
      const { error: schedErr } = await supabase
        ...
        .insert({ group_id: groupId, day_of_week: dayNum, start_time: assigned_time, ...(assigned_end_time ? { end_time: assigned_end_time } : {}) });

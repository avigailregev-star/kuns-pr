...
test('treats a legacy melodies group as a one-student private lesson', () => {
  const melodies = { ...own, lesson_type: 'melodies_group' };
  const cal = teacherCalendar(teacher, [row], [melodies], row);
  expect(cal.events).toEqual([]);
  const occupied = teacherCalendar(teacher, [{ ...row, student_name: 'שחר' }], [melodies], null);
  expect(occupied.events[0].label).toBe('שחר');
});

...
describe('syncRegistrationToAttendance — explicit group id (manual "existing lesson" pick)', () => {
  ...
  test('deactivates the student in their previous group when moved to an explicit group id', async () => {
    const mockSupabase = createMockSupabase({
      registrations: [{ data: [], error: null }, { error: null }],
      students: [
  ...
  test('keeps old group attendance active when another lesson still uses that group', async () => {
    const mockSupabase = createMockSupabase({
      registrations: [
        { data: [{ id: 'other', status: 'שובץ', registration_status: 'Pending' }], error: null },
        { error: null },
      ],
      students: [
    ...
    expect(mockSupabase.calls.some(c => c.table === 'students' && c.method === 'update' && c.payload?.is_active === false)).toBe(false);
  });
  ...
  test('creates a private attendance group for a melodies lesson', async () => {
    const mockSupabase = createMockSupabase({
      teachers: [{ data: { id: 't1' }, error: null }],
      groups: [{ data: [], error: null }, { data: { id: 'g-melodies' }, error: null }],
      group_schedules: [{ error: null }],
      registrations: [{ error: null }],
      students: [{ data: null, error: null }, { error: null }],
    });
    await syncRegistrationToAttendance(mockSupabase, baseReg({ selected_course: "מנגינות שנה ב' - עוד" }));
    const groupInsert = mockSupabase.calls.find(c => c.table === 'groups' && c.method === 'insert');
    expect(groupInsert.payload.lesson_type).toBe('melodies_individual');
  });

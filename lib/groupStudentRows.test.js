...
describe('groupStudentRows — category bucketing', () => {
  ...
  test('numbered fixed-lesson-group add-on names still bucket into theory/ensemble, not individual', () => {
    const individual = reg({ id: 'i1', selected_course: 'פסנתר 45 דקות' });
    const theoryAddon = reg({ id: 't1', selected_course: 'פיתוח קשב 1', linked_registration_id: 'i1' });
    const ensembleAddon = reg({ id: 'e1', selected_course: 'תזמורת כלי קשת 2', linked_registration_id: 'i1' });

    const groups = groupStudentRows([individual, theoryAddon, ensembleAddon]);
    expect(groups).toHaveLength(1);
    expect(groups[0].categories.theory).toEqual([theoryAddon]);
    expect(groups[0].categories.ensemble).toEqual([ensembleAddon]);
    expect(groups[0].categories.individual).toEqual([individual]);
  });

  test('add-ons use an assigned private lesson instead of an older empty registration', () => {
    const empty = reg({ id: 'empty', created_at: '2026-01-01T00:00:00Z', status: 'שובץ' });
    const assigned = reg({ id: 'assigned', created_at: '2026-01-02T00:00:00Z', status: 'שובץ', group_id: 'private-group', teacher: 'ילנה', assigned_day: 3, assigned_time: '17:00' });
    const [group] = groupStudentRows([empty, assigned]);
    expect(group.contactRow).toBe(empty);
    expect(group.addonSourceRow).toBe(assigned);
  });

  test('legacy theory labels with a level suffix stay in the theory category', () => {
    const individual = reg({ id: 'i1', selected_course: 'פרטני 45 דקות' });
    const legacyTheory = reg({ id: 't1', selected_course: "תיאוריה - רמה א׳", linked_registration_id: 'i1' });

    const groups = groupStudentRows([individual, legacyTheory]);

    expect(groups[0].categories.theory).toEqual([legacyTheory]);
    expect(groups[0].categories.individual).toEqual([individual]);
  });

  test('legacy choir label with a student suffix appears under ensemble', () => {
    const privateLesson = reg({ id: 'private', student_name: 'מרסלו', parent_phone: '050-3334445', selected_course: 'פרטני 45 דקות - מרסלו' });
    const choir = reg({ id: 'choir', student_name: 'מרסלו', parent_phone: '050-3334445', selected_course: 'מקהלה צעירה - מרסלו', group_id: null });
    const [group] = groupStudentRows([privateLesson, choir]);
    expect(group.categories.individual).toEqual([privateLesson]);
    expect(group.categories.ensemble).toEqual([choir]);
  });

  test('uses the linked group lesson_type for a custom ensemble name', () => {
    const individual = reg({ id: 'i1', selected_course: 'פסנתר 45 דקות' });
    const customEnsemble = reg({
      id: 'e1',
      selected_course: 'הרכב אפריקאי ייצוגי 1',
      linked_registration_id: 'i1',
      group_id: 'g-african',
    });

    const groups = groupStudentRows(
      [individual, customEnsemble],
      [{ id: 'g-african', lesson_type: 'orchestra' }]
    );

    expect(groups[0].categories.ensemble).toEqual([customEnsemble]);
    expect(groups[0].categories.individual).toEqual([individual]);
  });

  test('shows every stored lesson even when two rows point to the same group', () => {
    const first = reg({ id: 'e1', selected_course: 'שם ישן', group_id: 'g1' });
    const duplicate = reg({ id: 'e2', selected_course: 'הרכב א', group_id: 'g1', linked_registration_id: 'e1' });
    const groups = groupStudentRows([first, duplicate], [{ id: 'g1', name: 'הרכב א', lesson_type: 'orchestra' }]);
    expect(groups[0].categories.ensemble).toEqual([first, duplicate]);
  });

  test('keeps a private lesson visible if its stored group link is wrong', () => {
    const privateLesson = reg({ id: 'private', selected_course: 'פרטני 45 דקות', group_id: 'theory' });
    const groups = groupStudentRows([privateLesson], [{ id: 'theory', lesson_type: 'theory' }]);
    expect(groups[0].categories.individual).toEqual([privateLesson]);
  });

  test('keeps a legacy melodies lesson private even when linked to a theory group', () => {
    const melodies = reg({ id: 'melodies', selected_course: "מנגינות שנה ג' - בגלמה- אור מועלם", group_id: 'theory' });
    const groups = groupStudentRows([melodies], [{ id: 'theory', lesson_type: 'theory' }]);
    expect(groups[0].categories.individual).toEqual([melodies]);
    expect(groups[0].categories.theory).toEqual([]);
  });

  test('shows a linked melodies lesson under individual rather than ensemble', () => {
    const melodies = reg({ id: 'melodies', selected_course: "מנגינות שנה ב' - עוד- אור מועלם", group_id: 'melodies-group', status: 'שובץ', teacher: 'אורי לוי', assigned_day: 3, assigned_time: '16:15' });
    const groups = groupStudentRows([melodies], [{ id: 'melodies-group', lesson_type: 'melodies_group' }]);
    expect(groups[0].categories.individual).toEqual([melodies]);
    expect(groups[0].categories.ensemble).toEqual([]);
  });
});

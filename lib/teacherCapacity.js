...
export async function buildUsedMinutesMap(supabase) {
  ...
  const { data: groups } = await supabase
    .from('groups')
    .select('id, teacher_id, teachers(name, available_hours, teacher_availability_ranges(day_of_week, start_time, end_time)), group_schedules(day_of_week, start_time, end_time)')

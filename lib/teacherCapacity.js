...
export function getGroupLessonDuration(lessonType) {
  return (lessonType === 'individual_45' || lessonType === 'melodies_individual' || lessonType === 'melodies_group') ? 45 : 60;
}
...
export async function buildUsedMinutesMap(supabase) {
  ...
  const { data: assignments, error } = await supabase
    .from('registrations')
    .select('teacher, assigned_day, assigned_time, selected_course, status, registration_status, group_id')
  ...
  for (const group of (groups || [])) {
    ...
    const groupDuration = getGroupLessonDuration(group.lesson_type);

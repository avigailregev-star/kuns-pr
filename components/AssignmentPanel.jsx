    ...
    <div className={`border rounded-xl p-3 ${needsAttention(row) ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'}`}>
      ...
      <details className="mt-3 border-t pt-2">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3">
          ...
          {kind !== 'individual' && deleteRegistration && (
            <button type="button" onClick={() => deleteRegistration(row)} disabled={saving} className="text-xs px-3 py-2 rounded-lg border border-red-200 text-red-700 disabled:opacity-40">מחק שיעור</button>
          )}
        </div>
      </details>
